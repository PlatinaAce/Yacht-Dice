//역할: 싱글플레이를 위한 그래픽 인터페이스., 주사위 및 점수판
...
public class GameGUI {
    ...
    public void updateDiceForPlayer(int playerId, String diceResults) {
        if (playerId == 1) {
            // P1의 주사위 결과 업데이트
            // diceLabels 배열이나 관련 UI 요소 업데이트
        } else if (playerId == 2) {
            // P2의 주사위 결과 업데이트
        }
    }

    public void updateScoreForPlayer(int playerId, int row, int score) {
        if (playerId == 1) {
            tableModel.setValueAt(score, row, 1); // P1 열 업데이트
        } else if (playerId == 2) {
            tableModel.setValueAt(score, row, 2); // P2 열 업데이트
        }
    }
